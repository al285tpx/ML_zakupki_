...
class AnalyticsService:
    ...
    Ported to the backend for use with FastAPI and Celery.
    """

    # Singleton cache for the OKPD2 reference spreadsheet to avoid repeated disk I/O
    _okpd2_sprav_cache = None
    _last_sprav_path = None
    ...
    def _get_okpd2_sprav(self, path: str) -> Optional[pd.DataFrame]:
        ...
        Uses a class-level cache to implement the singleton pattern.
        """
        if AnalyticsService._okpd2_sprav_cache is not None and AnalyticsService._last_sprav_path == path:
            return AnalyticsService._okpd2_sprav_cache
        ...
        try:
            ...
            AnalyticsService._okpd2_sprav_cache = sprav_12
            AnalyticsService._last_sprav_path = path
            return AnalyticsService._okpd2_sprav_cache
        except Exception as e:
            logger.error(f"Error loading OKPD2 spreadsheet: {e}")
            return None

    def analyze_okpd2(self, df: pd.DataFrame, product_search: str, okpd2_sprav_path: str) -> Tuple[pd.DataFrame, List[str]]:
        """
        ...
        list_nan = ce_nan['product_name'].unique().tolist() if not ce_nan.empty else []
        ...
        list_nan2 = ce_nan2['product_name'].unique().tolist() if not ce_nan2.empty else []
    ...
    @staticmethod
    def classify_okpd2(df: pd.DataFrame, list_nan2: List[str], product_search: str) -> pd.DataFrame:
        """
